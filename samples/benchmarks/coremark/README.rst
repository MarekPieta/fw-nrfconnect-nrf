...
If you want to run the sample upon the startup, enable the :kconfig:option:`COREMARK_MODE_FLASH_AND_RUN` Kconfig option.
...
* The :kconfig:option:`CONFIG_COREMARK_MEMORY_METHOD_MALLOC` Kconfig option uses Zephyr :c:func:`k_malloc\k_free` functions to allocate the memory in the heap.
...
CoreMark can also be executed in the multithread mode.
...
#. To start the test, press the button assigned to the respective core.
  ...
  * All target cores work independently from each other, and it is possible to run the benchmark on several cores simultaneously.
    ...
    If the :kconfig:option:`COREMARK_MODE_FLASH_AND_RUN` Kconfig option is enabled, the measurement will launch automatically on all available cores when starting the application.
   ...
   The results will be similar to the following example:
      ...
      .. code-block:: console
         ...
         *** Booting Zephyr OS build bf606fc00ec1  ***
         ...
         CoreMark 1.0 : 163.304366 / GCC10.3.0 -O3 / STACK
