...
{
	__ASSERT_NO_MSG((power_state == POWER_STATE_OFF) || (power_state == POWER_STATE_ERROR_OFF));
	LOG_WRN("System turned off%s",
		(power_state == POWER_STATE_ERROR_OFF) ? " because of unrecoverable error" : "");
...
{
	struct power_off_event *event = new_power_off_event();
...
{
	set_power_state(POWER_STATE_ERROR_OFF);

	submit_power_off_event(true);
}
...
static bool app_event_handler(const struct app_event_header *aeh)
{
	...
	if (is_wake_up_event(aeh)) {
		...
		if ((power_state == POWER_STATE_OFF) ||
			...
			LOG_INF("Wake up while entering the system off - rebooting");
			sys_reboot(SYS_REBOOT_WARM);
		}
...
APP_EVENT_SUBSCRIBE_FINAL(MODULE, power_off_event);
