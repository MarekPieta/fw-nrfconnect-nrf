...
static int fp_storage_load_ak(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg)
{
	int rc;
	uint8_t index;
...
static int fp_settings_set(const char *name, size_t len, settings_read_cb read_cb, void *cb_arg)
{
	...
	if (!strncmp(name, key, sizeof(SETTINGS_AK_NAME_PREFIX) - 1)) {
		int err = fp_storage_load_ak(name, len, read_cb, cb_arg);
