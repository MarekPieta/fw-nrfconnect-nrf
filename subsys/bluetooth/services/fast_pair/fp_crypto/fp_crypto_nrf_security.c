...
static psa_key_id_t import_hmac_sha256_key(const uint8_t *data, size_t len)
{
	...
	psa_set_key_usage_flags(&key_attr, PSA_KEY_USAGE_VERIFY_HASH | PSA_KEY_USAGE_SIGN_HASH);
	...
	psa_status_t status = psa_import_key(&key_attr, data, len, &key_id);
			  ...
			  size_t hmac_key_len)
{
	...
	status = psa_mac_compute(hmac_key_id, PSA_ALG_HMAC(PSA_ALG_SHA_256), in, data_len,
	...
	if (olen != FP_CRYPTO_SHA256_HASH_LEN) {
		LOG_ERR("Invalid hmac sha 256 output length: %zu", olen);
		err = -EIO;
		goto cleanup;
	}
				 ...
				 const uint8_t *private_key)
{
	...
	uint8_t public_key_uncompressed[FP_CRYPTO_ECDH_PUBLIC_KEY_LEN +
					sizeof(uncompressed_format_marker)];
	int err = 0;
	uint32_t out_len = 0;
	...
	if (out_len != FP_CRYPTO_ECDH_SHARED_KEY_LEN) {
		LOG_ERR("Invalid psa_raw_key_agreement output len: %zu", out_len);
		err = -EIO;
	}
...
static int fp_crypto_nrf_security_init(void)
{
