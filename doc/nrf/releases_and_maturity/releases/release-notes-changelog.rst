...
* Updated:
  ...
  * Format version of :file:`dfu_application.zip` files generated by the NCS build system.
    Introducing Zephyr's :ref:`Hardware Model v2 <zephyr:hw_model_v2>` and building with :ref:`configuration_system_overview_sysbuild` for NCS 2.7 release modified content of the generated zip file.
    For example, binary file names and board name were updated.
    To indicate change of the zip format, ``format-version`` property in :file:`manifest.json` file included in the mentioned zip archive was updated from ``0`` to ``1``.
    Make sure that used DFU host tools support :file:`dfu_application.zip` file with the new format version (``1``).
    If update of the used DFU host tools is not possible, you can manually align the zip archive content with format version ``0``.
