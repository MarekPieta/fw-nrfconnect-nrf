				...
				void *param)
{
	...
	zassert_true(strnlen(key, DELETE_READOUT_MAX_NAME_LEN) < DELETE_READOUT_MAX_NAME_LEN);
	strcpy(name, key);
...
static int all_settings_delete(void)
{
	...
	while (true) {
		...
		zassert_not_equal(strcmp(name, prev_name), 0,
				  "Settings delete of a record failed to remove it");
...
static void teardown_fn(void)
{
	...
	zassert_ok(err, "Unable to erase settings");
