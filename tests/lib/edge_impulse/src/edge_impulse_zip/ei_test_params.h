...
/* Mocked results. */
static const char * const ei_classifier_labels[] = {"label1", "label2", "label3", "label4"};
#define EI_CLASSIFIER_LABEL_COUNT		ARRAY_SIZE(ei_classifier_labels)

/* Definitions for input and return values of the mocked library. */
#define EI_MOCK_GEN_FIRST_INPUT(PRED_IDX)	((float)((PRED_IDX) * EI_CLASSIFIER_RAW_SAMPLES_PER_FRAME))
#define EI_MOCK_GEN_LABEL_IDX(PRED_IDX)		((PRED_IDX) % EI_CLASSIFIER_LABEL_COUNT)
#define EI_MOCK_GEN_LABEL(PRED_IDX)		(ei_classifier_labels[EI_MOCK_GEN_LABEL_IDX(PRED_IDX)])
