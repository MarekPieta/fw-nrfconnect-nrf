...
{
	__ASSERT_NO_MSG((val_cnt == DEFAULT_CHAN_COUNT) || (chan == SENSOR_CHAN_ACCEL_XYZ));
...
static int generate_accel_data(enum sensor_channel chan)
{
	...
	switch (chan) {
	case SENSOR_CHAN_ACCEL_X:
		retval = gen_fn(chan, DEFAULT_CHAN_COUNT, &accel_samples[0]);
		break;
	case SENSOR_CHAN_ACCEL_Y:
		retval = gen_fn(chan, DEFAULT_CHAN_COUNT, &accel_samples[1]);
		break;
	case SENSOR_CHAN_ACCEL_Z:
		retval = gen_fn(chan, DEFAULT_CHAN_COUNT, &accel_samples[2]);
		break;
	case SENSOR_CHAN_ACCEL_XYZ:
