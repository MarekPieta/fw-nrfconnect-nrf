			...
			bool send_always)
{
	...
	if (!check_state || (rs->state != STATE_DISCONNECTED)) {
		unsigned int pipeline_depth = UINT_MAX;
		...
		pipeline_depth = ((pipeline_depth < rs->subscriber->pipeline_size) ?
				  pipeline_depth : rs->subscriber->pipeline_size);
...
static uint8_t get_subscriber_priority(const struct subscriber *sub)
{
	return sub->priority;
}
						   ...
						   struct report_state *former_rs)
{
	...
	for (size_t i = 0; i < ARRAY_SIZE(state.subscriber); i++) {
		...
		uint8_t sub_prio = get_subscriber_priority(sub);
...
static void connect(const void *subscriber_id, uint8_t report_id)
{
	...
	if (rd->linked_rs) {
		...
		uint8_t cur_sub_prio = get_subscriber_priority(rd->linked_rs->subscriber);
		uint8_t new_sub_prio = get_subscriber_priority(subscriber);
