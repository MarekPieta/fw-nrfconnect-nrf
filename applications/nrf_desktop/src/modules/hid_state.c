...
static void send_report_keyboard(struct report_state *rs, struct report_data *rd)
{
	...
	for (size_t i = 0; i < key_cnt; i++) {
		...
		if (usage_id <= KEYBOARD_REPORT_LAST_KEY) {
			__ASSERT_NO_MSG(usage_id <= UINT8_MAX);
			key_data_ptr[cnt] = usage_id;
			cnt++;
			...
			LOG_ERR("Undefined usage 0x"PRIx16, usage_id);
		}
	}

	/* Fill the rest of report with zeros. */
	memset(&key_data_ptr[cnt], 0x00, KEYBOARD_REPORT_KEY_COUNT_MAX - cnt);
...
static uint8_t get_mouse_button_bm(struct report_data *rd)
{
	uint16_t keys[MOUSE_REPORT_KEY_COUNT_MAX];
	size_t key_cnt = keys_state_keys_get(&rd->keys_state, keys, ARRAY_SIZE(keys));
	uint8_t button_bm = 0;
...
static void send_report_mouse(struct report_state *rs, struct report_data *rd)
{
	...
	event->dyndata.data[0] = rs->report_id;
	event->dyndata.data[1] = get_mouse_button_bm(rd);
...
static bool update_report(struct report_data *rd)
{
	...
	while (!update_needed) {
		uint16_t usage_id;
		bool pressed;
		int err = hid_eventq_keypress_get(&rd->eventq, &usage_id, &pressed);
		...
		err = keys_state_key_update(ks, usage_id, pressed, &update_needed);
		/* It's expected to never exceed the maximum number of active keys. */
		__ASSERT_NO_MSG(!err);
...
static void connect(const void *subscriber_id, uint8_t report_id)
{
	...
	if (!rd->linked_rs) {
		...
		if (!hid_eventq_is_empty(&rd->eventq)) {
			...
			uint32_t min_ts = k_uptime_get_32();

			min_ts = ((min_ts >= CONFIG_DESKTOP_HID_REPORT_EXPIRATION) ?
				 (min_ts - CONFIG_DESKTOP_HID_REPORT_EXPIRATION) : (0));
...
static void update_key(const struct hid_keymap *map, bool pressed)
{
	...
	if (!connected || !hid_eventq_is_empty(&rd->eventq)) {
		int err = hid_eventq_keypress_enqueue(&rd->eventq, map->usage_id, pressed);
