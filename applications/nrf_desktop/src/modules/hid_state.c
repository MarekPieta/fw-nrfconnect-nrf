...
struct provider {
	const struct hid_report_provider_fops *fops;
					     ...
					     uint8_t report_id)
{
	...
	size_t pos;
	for (pos = 0; pos < ARRAY_SIZE(input_reports); pos++) {
		if (input_reports[pos] == report_id) {
...
static struct subscriber *get_subscriber(const void *subscriber_id)
{
	...
	LOG_WRN("No subscriber %p", (void *)subscriber_id);
	return NULL;
}

static struct provider *get_provider(uint8_t report_id)
{
	for (size_t i = 0; i < ARRAY_SIZE(state.provider); i++) {
			...
			bool send_always)
{
	...
	while ((rs->cnt < pipeline_depth) &&
		...
		if (provider->linked_rs != rs) {
			if (provider->fops->send_empty_report) {
				report_sent = provider->fops->send_empty_report(provider->report_id,
										rs->update_needed,
										&state,
										rs->subscriber->id);
			} else {
				/* Unsupported. Mark the report as sent, do not retry. */
				report_sent = true;
			}
		} else {
			report_sent = provider->fops->send_report(provider->report_id,
								  rs->update_needed,
								  &state,
								  rs->subscriber->id);
		}
...
static void link_provider_to_rs(struct provider *provider, struct report_state *rs)
{
	...
	provider->fops->connection_state(provider->report_id, (rs != NULL));

	if (rs) {
		/* Refresh state of the newly linked subscriber. */
		report_send(rs, NULL, true);
	} else if (prev_rs->state != STATE_DISCONNECTED) {
		/* Provide empty HID report to the previous subscriber to clear state. */
		report_send(prev_rs, NULL, true);
	}
}
...
static void report_issued(const void *subscriber_id, uint8_t report_id, bool error)
{
	...
	if (rs->state != STATE_DISCONNECTED) {
		...
		if (error && (rs->provider->linked_rs == rs)) {
			...
			link_provider_to_rs(provider, prev_rs);
			return;
		}
	}
...
static void link_subscriber_reports(struct subscriber *subscriber)
{
...
static void unlink_subscriber_reports(struct subscriber *subscriber)
{
	for (size_t i = 0; i < ARRAY_SIZE(subscriber->state); i++) {
		struct report_state *rs = &subscriber->state[i];
			       ...
			       uint8_t pipeline_size, uint8_t report_max)
{
	...
	if (!state.active_subscriber) {
		...
		__ASSERT_NO_MSG(state.active_subscriber->priority !=
				sub->priority);
		unlink_subscriber_reports(state.active_subscriber);

		state.active_subscriber = sub;
	}

	update_output_report_state();
	LOG_INF("Subscriber %p connected", (void *)subscriber_id);
}
...
static void disconnect_subscriber(const void *subscriber_id)
{
	...
	if (sub == state.active_subscriber) {
		unlink_subscriber_reports(sub);

		state.active_subscriber = find_next_subscriber(sub);

		if (state.active_subscriber) {
			link_subscriber_reports(state.active_subscriber);
		}
...
static int hid_state_report_trigger(uint8_t report_id)
{
	...
	if (provider->linked_rs) {
		if(!report_send(provider->linked_rs, provider, true)) {
			err = -EBUSY;
		}
	} else {
...
static bool handle_hid_report_provider_event(struct hid_report_provider_event *event)
{
	...
	__ASSERT_NO_MSG(event->p_fops);
	__ASSERT_NO_MSG(event->p_fops->send_report);
	__ASSERT_NO_MSG(event->p_fops->connection_state);

	provider->report_id = event->report_id;
	provider->fops = event->p_fops;

	/* Provide function pointers to let provier trigger sending subsequent HID report. */
	__ASSERT_NO_MSG(!event->hs_fops);
	event->hs_fops = &hid_state_fops;
