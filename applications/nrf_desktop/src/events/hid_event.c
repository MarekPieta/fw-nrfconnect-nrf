						...
						const struct app_event_header *aeh)
{
	...
	nrf_profiler_log_encode_uint8(buf, event->connected ? event->params.priority : 0);
	nrf_profiler_log_encode_uint8(buf, event->connected ? event->params.pipeline_size : 0);
	nrf_profiler_log_encode_uint8(buf, event->connected ? event->params.report_max : 0);
}
