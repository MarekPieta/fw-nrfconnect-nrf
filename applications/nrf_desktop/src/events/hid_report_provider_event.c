...
static void log_hid_report_provider_event(const struct app_event_header *aeh)
{
	...
	APP_EVENT_MANAGER_LOG(aeh, "Report ID:0x%" PRIx8 ", provider fops:%p, HID state fops:%p",
			      event->report_id,
			      (const void *)event->p_fops, (const void *)event->hs_fops);
}
					      ...
					      const struct app_event_header *aeh)
{
	...
	BUILD_ASSERT(sizeof(uint32_t) >= sizeof(event->p_fops));
	nrf_profiler_log_encode_uint32(buf, (uint32_t)event->p_fops);
	BUILD_ASSERT(sizeof(uint32_t) >= sizeof(event->hs_fops));
	nrf_profiler_log_encode_uint32(buf, (uint32_t)event->hs_fops);
}

APP_EVENT_INFO_DEFINE(hid_report_provider_event,
		      ENCODE(NRF_PROFILER_ARG_U8, NRF_PROFILER_ARG_U32, NRF_PROFILER_ARG_U32),
		      ENCODE("report_id", "p_fops", "hs_fops"),
