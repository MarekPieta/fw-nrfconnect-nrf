...
int hid_eventq_keypress_enqueue(struct hid_eventq *q, uint16_t id, bool pressed)
{
...
int hid_eventq_keypress_get(struct hid_eventq *q, uint16_t *id, bool *pressed)
{
...
void hid_eventq_cleanup(struct hid_eventq *q, uint32_t min_timestamp)
{
	...
	SYS_SLIST_FOR_EACH_NODE_SAFE(&q->root, cur_node, tmp_safe) {
		...
		if (cur_evt->data.pressed) {
			...
			if (release_node && (release_pos > max_pos)) {
