...
#. The :ref:`CONFIG_DESKTOP_HID_FORWARD_ENABLE <config_desktop_app_options>` option selects :ref:`CONFIG_DESKTOP_HID_REPORTQ <config_desktop_app_options>` to automatically enable the :ref:`nrf_desktop_hid_reportq`.
   The HID report queue utility is used to locally enqueue reports at the source to prevent HID report drops.
   If needed, you can align maximum number of enqueued HID reports (:ref:`CONFIG_DESKTOP_HID_REPORTQ_MAX_ENQUEUED_REPORTS <config_desktop_app_options>`).
   See HID report queue utility documentation for details.
...
In the current implementation, the :ref:`nrf_desktop_usb_state` supports either the HID boot keyboard or the HID boot mouse reports, because the application does not support assigning HID boot protocol code separately per USB HID instance.

The :c:func:`hogp_read` callback is called when HID input report is received from the connected peripheral.
The received HID input report data is added to the HID report queue and converted to :c:struct:`hid_report_event`.

The :c:struct:`hid_report_event` is submitted by the HID report queue and then the HID-class USB device configured by :ref:`nrf_desktop_usb_state` forwards it to the host.
...
USB HID class instances limit maximum number of HID input reports that can be handled simultaneously.
The number of HID input reports received from HID peripherals connected over Bluetooth LE may exceed the limit.
If needed, the |hid_forward| relies on HID report queue to locally enqueue HID input reports before providing them to USB HID subscriber.
All of the HID input reports received from a HID peripheral go through the HID report queue associated with a given USB HID instance.
