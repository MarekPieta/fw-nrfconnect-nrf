...
#. The |ble_scan| selects the following configuration options:
...
#. Make sure that the number of scan filters based on the Bluetooth address (:kconfig:option:`CONFIG_BT_SCAN_ADDRESS_CNT`) value must be equal to the number of Bluetooth bonds.
...
#.Make sure that the number of scan filters based on the Bluetooth name (:kconfig:option:`CONFIG_BT_SCAN_NAME_CNT`) value is equal to the number of peripheral types the nRF Desktop central connects to.
...
#. If you want to limit the number of attempts to connect to a device, you can use the connection attempt filter with the :kconfig:option:`CONFIG_BT_SCAN_CONN_ATTEMPTS_FILTER` Kconfig option.
