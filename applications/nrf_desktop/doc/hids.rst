...
1. Complete the basic Bluetooth configuration, as described in :ref:`nrf_desktop_bluetooth_guide`.
   Make sure that both :ref:`COFNIG_DESKTOP_ROLE_HID_PERIPHERAL <config_desktop_app_options>` and :ref:`CONFIG_DESKTOP_BT_PERIPHERAL <config_desktop_app_options>` are enabled.
   The HID Service application module is enabled by :ref:`CONFIG_DESKTOP_HIDS_ENABLE <config_desktop_app_options>`, which is selected by the :ref:`CONFIG_DESKTOP_BT_PERIPHERAL <config_desktop_app_options>` together with other GATT Services that are required for a HID device.
#. The ref:`CONFIG_DESKTOP_HIDS_ENABLE <config_desktop_app_options>` selects kconfig:option:`CONFIG_BT_HIDS` to automatically enable the :ref:`hids_readme`.
