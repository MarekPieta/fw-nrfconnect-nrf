...
.. note::
   Since NCS 2.7 release, format version of the :file:`dfu_application.zip` file generated by the build system was updated from ``0`` to ``1``.
   The format version ``1`` support was also added to the script in NCS 2.7 release.
   Make sure to use updated version of the script for archives in the new format version.
   The new version of the script can also handle archives in legacy format version.

   If update of the used script version is not possible, you can manually align the zip archive content with format version ``0``.
