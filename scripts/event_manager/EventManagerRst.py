...
class EventManagerRst():
    ...
    def _gen_event_propagation_array(em_info, listener):
        ...
        # Hotfix for case when module is twice in the dict (various module implementations have
        # various priority for given event or Kconfig configurable subscription priority).
        in_events['EARLY'] = list(set(in_events['EARLY']) - set(in_events['FIRST']))
        in_events['EARLY'].sort()

        in_events[''] = list(set(in_events['']) - set(in_events['FIRST']))
        in_events[''] = list(set(in_events['']) - set(in_events['EARLY']))
        in_events[''].sort()

        in_events['FINAL'] = list(set(in_events['FINAL']) - set(in_events['FIRST']))
    ...
    def save_all_event_related_modules(self, module_limit=None):
        ...
        for evt in events:
            ...
            # Hotfix for case when module is twice in the dict (various module implementations have
            # various priority for given event or Kconfig configurable subscription priority).
            sinks_dict['EARLY'] = list(set(sinks_dict['EARLY']) - set(sinks_dict['FIRST']))
            sinks_dict['EARLY'].sort()

            sinks_dict[''] = list(set(sinks_dict['']) - set(sinks_dict['FIRST']))
            sinks_dict[''] = list(set(sinks_dict['']) - set(sinks_dict['EARLY']))
            sinks_dict[''].sort()

            sinks_dict['FINAL'] = list(set(sinks_dict['FINAL']) - set(sinks_dict['FIRST']))
