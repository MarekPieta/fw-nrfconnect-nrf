...
class EventManagerRst():
    def __init__(self, em_app_info, app_dir):
        ...
        self.out_dir = os.path.join(app_dir, "generated")
    ...
    def save_all_event_propagation_arrays(self):
        evt_prop_out_dir = os.path.join(self.out_dir, "event_propagation")
        if not os.path.exists(evt_prop_out_dir):
            os.mkdir(evt_prop_out_dir)

        listeners = self.em_app_info.get_all_listeners()

        for l in listeners:
            arr = EventManagerRst._gen_event_propagation_array(self.em_app_info, l)
            with open(os.path.join(evt_prop_out_dir, l + '.rst'), 'w+') as f:
                f.write(arr)
                f.write('\n')

    def save_all_event_related_modules(self, module_limit=None):
        evt_rel_modules_out_dir = os.path.join(self.out_dir, "event_rel_modules")

        if not os.path.exists(evt_rel_modules_out_dir):
            os.mkdir(evt_rel_modules_out_dir)
        ...
        for evt in events:
            ...
            with open(os.path.join(evt_rel_modules_out_dir, evt + '.rst'), 'w+') as f:
                f.write('.. _{}_{}_{}:\n\n'.format(PROJECT_NAME, evt, 'sources'))
                f.write('Source modules for the ``{}``:\n\n'.format(evt))
                for m in sources:
                    f.write('* :ref:`{}_{}`\n'.format(PROJECT_NAME, m))

                f.write('\n')
                f.write('.. _{}_{}_{}:\n\n'.format(PROJECT_NAME, evt, 'sinks'))
                f.write('Sink modules for the ``{}``:\n\n'.format(evt))
                for m in sinks:
                    f.write('* :ref:`{}_{}`\n'.format(PROJECT_NAME, m))
                f.close()
